...
class CoverageSummaryGenerator:
    ...
    def __init__(self):
        ...
        self.frontend_coverage = self.project_root / 'frontend' / 'coverage' / 'coverage-final.json'
        ...
        try:
            ...
            for package in root.findall('.//package'):
                for class_elem in package.findall('.//class'):
                    ...
                    if any(critical in filename for critical in [
                        'services/ecg/', 'services/diagnosis/', 
                        'utils/medical/', 'core/medical_safety'
                    ]):
                        file_line_rate = float(class_elem.get('line-rate', 0)) * 100
                        file_branch_rate = float(class_elem.get('branch-rate', 0)) * 100
                        critical_components[filename] = (file_line_rate + file_branch_rate) / 2
        ...
        try:
            ...
            for file_path, file_data in data.items():
                ...
                if any(critical in file_path for critical in [
                    'components/medical/', 'services/ecg/', 
                    'services/diagnosis/', 'utils/medical/'
                ]):
            ...
            return {
                'total': (lines_pct + branches_pct + functions_pct + statements_pct) / 4,
                'lines': lines_pct,
    ...
    def check_coverage_requirements(self, backend: Dict, frontend: Dict) -> Dict[str, bool]:
        ...
        requirements = {
            ...
            'backend_critical': all(cov >= 100.0 for cov in backend['critical_components'].values()),
            'frontend_critical': all(cov >= 100.0 for cov in frontend['critical_components'].values()),
            'ml_service': True,  # Verificar especificamente o ML service
        }
        
        # Verificar ML Service especificamente
        ml_service_coverage = next(
            (cov for path, cov in backend['critical_components'].items() 
...
"""
        ...
        if not all_passed:
            ...
            if not requirements['backend_critical']:
                failed_critical = [
                    path for path, cov in backend['critical_components'].items() 
                    if cov < 100.0
                ]
            ...
            if not requirements['frontend_critical']:
                failed_critical = [
                    path for path, cov in frontend['critical_components'].items() 
                    if cov < 100.0
                ]
