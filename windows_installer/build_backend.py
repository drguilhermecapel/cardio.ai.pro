...
def check_environment():
    ...
    if sys.version_info < (3, 8):
        checks.append(
            ("Python Version", "FAIL", f"Python 3.8+ required, found {sys.version}")
        )
    else:
        checks.append(
            (
    ...
    if not Path("../backend").exists():
        checks.append(
            ("Backend Directory", "FAIL", "Backend directory not found at ../backend")
        )
    else:
        ...
        if free_space < 2:
            checks.append(
                (
                    "Disk Space",
                    "FAIL",
                    f"Insufficient disk space: {free_space:.1f}GB (2GB required)",
                )
            )
        else:
...
def install_poetry_with_retry():
    ...
    for attempt in range(max_retries):
        try:
            print(
                f"⚠️ Poetry not found, installing... (attempt {attempt + 1}/{max_retries})"
            )
            ...
            error_msg = (
                f"Poetry installation timed out after 5 minutes (attempt {attempt + 1})"
            )
            print(f"❌ {error_msg}")
            ...
            if attempt < max_retries - 1:
                ...
                print(
                    "2. Install Poetry manually: https://python-poetry.org/docs/#installation"
                )
            ...
            if attempt < max_retries - 1:
                ...
                print("SOLUTIONS:")
                print(
                    "1. Install Poetry manually: https://python-poetry.org/docs/#installation"
                )
...
def setup_environment():
    ...
    with build_phase("Environment Setup"):
        ...
        try:
            ...
            raise BuildError(
                "PyInstaller installation timeout", phase="pyinstaller_install"
            )
        except subprocess.CalledProcessError as e:
            print(f"❌ ERROR: Failed to install PyInstaller: {e}")
            raise BuildError(
                "PyInstaller installation failed", phase="pyinstaller_install"
            )
...
def build_executable():
    ...
    with build_phase("Executable Build"):
        ...
        if exe_file.exists():
            ...
            exe_size = (installer_dir / "cardioai-backend.exe").stat().st_size / (
                1024 * 1024
            )  # MB
            print(
                f"✅ Executable copied to: {installer_dir / 'cardioai-backend.exe'} ({exe_size:.1f} MB)"
            )
            ...
            raise BuildError(
                "Executable not found after build", phase="executable_deployment"
            )
...
def validate_executable():
    ...
    try:
        result = subprocess.run(
            [str(exe_file), "--help"], capture_output=True, text=True, timeout=30
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Build CardioAI Pro Backend for Windows"
    )
    parser.add_argument(
        "--debug",
        "-d",
