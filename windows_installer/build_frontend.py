...
def check_environment():
    ...
    try:
        ...
        if version_num < 16:
            checks.append(("Node.js Version", "FAIL", f"Node.js 16+ required, found {node_version}"))
        else:
    ...
    if not Path("../frontend").exists():
        checks.append(("Frontend Directory", "FAIL", "Frontend directory not found at ../frontend"))
    else:
    ...
    if frontend_dir.exists() and not (frontend_dir / "package.json").exists():
        checks.append(("package.json", "FAIL", "package.json not found in frontend directory"))
    ...
    try:
        ...
        if free_space < 1:
            checks.append(("Disk Space", "FAIL", f"Insufficient disk space: {free_space:.1f}GB (1GB required)"))
        else:
...
def install_dependencies_with_progress():
    ...
    with build_phase("Frontend Dependencies Installation"):
        ...
        try:
            ...
            print("4. Try using a different npm registry: npm config set registry https://registry.npmjs.org/")
            ...
            raise BuildError("Unexpected dependency installation error", phase="dependencies")
...
def install_with_npm():
    ...
    while True:
        ...
        if output == '' and process.poll() is not None:
            break
        if output:
            if any(keyword in output.lower() for keyword in ['warn', 'error', 'installing', 'added', 'updated']):
...
def install_with_yarn():
    ...
    try:
        subprocess.run(["yarn", "--version"], check=True, capture_output=True, timeout=10)
...
def install_with_pnpm():
    ...
    try:
        subprocess.run(["pnpm", "--version"], check=True, capture_output=True, timeout=10)
    ...
    with build_phase("Frontend Production Build"):
        ...
        try:
            ...
            if not dist_dir.exists():
                raise BuildError("Build output directory 'dist' not found", phase="build_verification")
            ...
            build_size = sum(f.stat().st_size for f in dist_dir.rglob('*') if f.is_file()) / (1024*1024)  # MB
...
def build_with_npm():
    ...
    while True:
        ...
        if output == '' and process.poll() is not None:
            break
        if output:
            if any(keyword in output.lower() for keyword in ['building', 'compiled', 'error', 'warning', 'done']):
                print(f"   {output.strip()}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Build CardioAI Pro Frontend for Windows')
