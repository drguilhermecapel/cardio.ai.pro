...
def get_conditions_by_category(category: SCPCategory) -> list[SCPCondition]:
    ...
    return [
        condition
        for condition in SCP_ECG_CONDITIONS.values()
...
def get_critical_conditions() -> list[SCPCondition]:
    ...
    return [
        SCP_ECG_CONDITIONS[code]
        for code in critical_codes
        if code in SCP_ECG_CONDITIONS
    ]
...
def get_conditions_by_urgency(urgency: str) -> list[SCPCondition]:
    ...
    return [
        SCP_ECG_CONDITIONS[code] for code in urgency_codes if code in SCP_ECG_CONDITIONS
    ]
...
def validate_scp_conditions() -> dict[str, Any]:
    ...
    validation_results = {
        ...
        "urgency_levels": len(
            {c.clinical_urgency for c in SCP_ECG_CONDITIONS.values()}
        ),
        "conditions_with_icd10": len(
            [c for c in SCP_ECG_CONDITIONS.values() if c.icd10_codes]
        ),
        "high_sensitivity_conditions": len(
            [c for c in SCP_ECG_CONDITIONS.values() if c.sensitivity_target >= 0.95]
        ),
