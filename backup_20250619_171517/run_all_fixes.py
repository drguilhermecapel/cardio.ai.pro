...
def main():
    ...
    success = run_command(
        "python app/core/fix_config_constants.py",
    ...
    success = run_command(
        "python app/api/v1/endpoints/fix_all_endpoints.py", "Fixing API endpoints"
    )
    if not success:
    ...
    for test_file in critical_test_files:
        if Path(test_file).exists():
            run_command(
                f"pytest {test_file} -v --cov=app --cov-append", f"Running {test_file}"
            )
    ...
    run_command("coverage report -m", "Generating coverage report")
    ...
    for module in critical_modules:
        run_command(f"coverage report -m --include={module}", f"Coverage for {module}")
