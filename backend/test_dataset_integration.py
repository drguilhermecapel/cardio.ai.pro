...
def test_dataset_service_integration():
    ...
    try:
        ...
        for dep, status in dependencies.items():
            ...
            print(f"  {status_icon} {dep}: {'Disponível' if status else 'Não disponível'}")
        ...
        for name, info in datasets_info.items():
            print(f"  • {name}: {info['records']} registros, {info['sampling_rate']} Hz")
        ...
        if hasattr(hybrid_service, 'dataset_service_available'):
            if hybrid_service.dataset_service_available:
                ...
                print(f"✓ Dataset service: {type(hybrid_service.dataset_service).__name__}")
            else:
                print("⚠ Serviço de datasets não disponível no HybridECGAnalysisService")
        else:
