    ...
    def parse_coverage_xml(self) -> Dict[str, float]:
        ...
        for package in root.findall('.//package'):
            for class_elem in package.findall('.//class'):
                ...
                is_critical = any(critical in filename for critical in self.CRITICAL_PATHS)
    ...
    def generate_improvement_report(self) -> Dict[str, any]:
        ...
        report = {
            ...
            'summary': {
                ...
                'critical_files': sum(1 for m in self.metrics.values() if m.is_critical),
        ...
        for path, metrics in self.metrics.items():
            ...
            if coverage < target:
                ...
                priority = 'CRITICAL' if metrics.is_critical else 'HIGH' if coverage < 60 else 'MEDIUM'
        ...
        report['priorities'].sort(key=lambda x: (
            x['priority'] == 'CRITICAL',
            x['gap']
        ), reverse=True)
...
def main():
    ...
    parser.add_argument(
        '--analyze',
        action='store_true',
        help='Analisar cobertura atual'
    )
    parser.add_argument(
        '--run',
        action='store_true',
        help='Executar testes com cobertura'
    )
    parser.add_argument(
        '--module',
        help='Módulo específico para análise'
    )
    parser.add_argument(
        '--generate-stubs',
        help='Gerar stubs de teste para arquivo'
    )
    parser.add_argument(
        '--export-report',
        help='Exportar relatório para diretório'
    )
    parser.add_argument(
        '--improvement-plan',
        action='store_true',
        help='Gerar plano de melhoria'
    )
    ...
    if args.improvement_plan:
        ...
        print(f"  - Total de linhas não cobertas: {report['summary']['total_missing_lines']}")
        
        print(f"\nPrioridades:")
        for i, priority in enumerate(report['priorities'][:10], 1):
            print(f"\n{i}. {priority['file']}")
            print(f"   Prioridade: {priority['priority']}")
            print(f"   Cobertura: {priority['current_coverage']:.1f}% → {priority['target_coverage']:.1f}%")
        ...
        # Salvar relatório
        report_path = project_root / f'coverage_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    ...
    if args.generate_stubs:
        ...
        if stub:
            stub_path = project_root / f'tests/test_{Path(args.generate_stubs).stem}_coverage.py'
