...
async def test_async_service_methods():
    ...
    try:
        ...
        ecg_service = ECGAnalysisService(mock_db, mock_ml_service, mock_validation_service, mock_notification_service)
        assert ecg_service is not None
        
        validation_service = ValidationService(mock_db, mock_notification_service)
        assert validation_service is not None
        
        ecg_service.repository.get_analysis_by_id = AsyncMock(side_effect=Exception("DB error"))
        try:
