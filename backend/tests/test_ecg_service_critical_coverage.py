...
class TestECGServiceCriticalPaths:
    ...
    @pytest.mark.asyncio
    async def test_create_analysis_complete_flow(self, ecg_service, mock_dependencies, sample_ecg_data):
        """Test complete ECG analysis creation flow"""
        # Setup mocks
        mock_patient = Mock(id=1, patient_id='TEST123')
        mock_dependencies['patient_service'].get_patient_by_patient_id.return_value = mock_patient
        ...
        mock_dependencies['notification_service'].send_analysis_complete.assert_called_once()
        ...
        for file_type, data in formats_data.items():
            with patch('app.services.ecg_service.ECGProcessor') as MockProcessor:
                ...
                result = await ecg_service._process_ecg_file(
                    file_path=f'/test/ecg.{file_type.value}',
                    file_type=file_type
                )
    ...
    async def test_validate_signal_quality_comprehensive(self, ecg_service):
        ...
        for case in test_cases:
            ...
            if case['expected_issues']:
                assert any(issue in result['issues'] for issue in case['expected_issues'])
    ...
    async def test_error_handling_and_recovery(self, ecg_service, mock_dependencies):
        ...
        mock_dependencies['ml_service'].predict_comprehensive.side_effect = Exception("ML Error")
        mock_dependencies['multi_pathology_service'].analyze_hierarchical.return_value = {
            'diagnosis': 'UNKNOWN',
            'confidence': 0.5,
        ...
        mock_dependencies['multi_pathology_service'].analyze_hierarchical.assert_called_once()
    ...
    async def test_concurrent_analysis_processing(self, ecg_service, mock_dependencies):
        ...
        assert mock_dependencies['ecg_repo'].update_analysis.call_count >= num_concurrent
    ...
    @pytest.mark.asyncio
    async def test_report_generation_comprehensive(self, ecg_service, mock_dependencies):
        ...
        mock_dependencies['interpretability_service'].generate_comprehensive_explanation.return_value = Mock(
    ...
    async def test_signal_preprocessing_pipeline(self, ecg_service):
        ...
        # Preprocess
        with patch('app.preprocessing.advanced_pipeline.AdvancedECGPreprocessor') as MockPreprocessor:
            ...
            service = ECGAnalysisService(
                db=ecg_service.db,
                ecg_repository=ecg_service.ecg_repository
            )
...
class TestECGServiceMedicalCompliance:
    ...
    async def test_clinical_decision_support(self, ecg_service):
        ...
        for scenario in test_scenarios:
            recommendations = await ecg_service._generate_medical_recommendations(
                ...
                urgency=ClinicalUrgency.HIGH if scenario['diagnosis'] != 'NORMAL' else ClinicalUrgency.LOW
            )
            
            # Verify appropriate recommendations
            assert len(recommendations) > 0
            if scenario['diagnosis'] == 'STEMI':
                assert any('catheterization' in r.lower() or 'stemi' in r.lower() 
    ...
    async def test_audit_trail_generation(self, ecg_service, mock_dependencies):
        ...
        with patch('app.core.logging.audit_logger') as mock_audit:
            ...
            # Create analysis
            mock_dependencies['patient_service'].get_patient_by_patient_id.return_value = Mock(id=1)
            mock_dependencies['ecg_repo'].create_analysis.return_value = Mock(
                id=1,
                analysis_id='AUDIT123',
                status=AnalysisStatus.COMPLETED
            )
