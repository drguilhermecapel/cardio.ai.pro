        ...
        for i in range(0, len(t), beat_samples):
            if i + 100 < len(t):
                ...
                if p_start > 0 and p_start + int(0.04*fs) < len(ecg):
                    ...
                    if p_wave_samples > 0:
                        p_wave = 0.2 * signal.windows.gaussian(p_wave_samples, std=fs*0.01)
                        ecg[p_start:p_start+p_wave_samples] += p_wave[:p_wave_samples]
                
                # Complexo QRS (pico R)
                qrs_width = int(0.08 * fs)
                if i + qrs_width <= len(ecg) and qrs_width > 0:
                    qrs_signal = signal.windows.gaussian(qrs_width, std=qrs_width/6) * 1.5
        ...
        if pathology == "afib":
            ...
            for i in range(0, len(t), beat_interval):
                if i + 60 < len(t):
                    ...
                    if i + qrs_width <= len(ecg) and qrs_width > 0:
                        qrs_signal = signal.windows.gaussian(qrs_width, std=qrs_width/4) * 2
                        ecg[i:i+qrs_width] += qrs_signal
            ...
            for i in range(len(ecg)):
                if i % int(60/72 * fs) > int(0.1 * fs) and i % int(60/72 * fs) < int(0.25 * fs):
...
class TestECGPreprocessing(unittest.TestCase):
    ...
    def test_quality_metrics_accuracy(self):
        ...
        for ecg_type, ecg_signal in [("clean", ecg_clean), 
                                     ("noisy", ecg_noisy), 
                                     ("drift", ecg_drift)]:
