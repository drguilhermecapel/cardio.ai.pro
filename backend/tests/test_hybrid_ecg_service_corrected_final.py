...
Targeting 95%+ Coverage with Corrected Method Signatures
...
class TestUniversalECGReaderCorrected:
    """Comprehensive tests for UniversalECGReader with corrected signatures."""
    ...
    def test_read_mitbih_error_fallback_corrected(self, ecg_reader, tmp_path):
        """Test MIT-BIH format error handling with CSV fallback - CORRECTED."""
        with patch('app.services.hybrid_ecg_service.wfdb') as mock_wfdb:
    ...
    def test_file_not_found_error_corrected(self, ecg_reader):
        """Test error handling for non-existent files - CORRECTED."""
        result = ecg_reader.read_ecg('nonexistent.csv')
        assert result is None  # Expected behavior for non-existent files
    ...
    def test_read_edf_import_error_fallback_corrected(self, ecg_reader, tmp_path):
        """Test EDF import error fallback - CORRECTED."""
...
class TestFeatureExtractorCorrected:
    """Comprehensive tests for FeatureExtractor with corrected signatures."""
    ...
    def test_detect_r_peaks_corrected(self, feature_extractor):
        """Test R peak detection - CORRECTED signature."""
        test_signal = np.random.randn(1000, 1) * 0.1
    ...
    def test_interval_features_corrected(self, feature_extractor):
        """Test interval feature extraction - CORRECTED."""
        test_signal = np.random.randn(1000, 1) * 0.1
        r_peaks = np.array([100, 300, 500, 700, 900])
    ...
    def test_hrv_features_corrected(self, feature_extractor):
        """Test HRV feature extraction - CORRECTED signature."""
        r_peaks = np.array([100, 300, 500, 700, 900])
...
class TestHybridECGAnalysisServiceCorrected:
    """Comprehensive tests for HybridECGAnalysisService with corrected signatures."""
    ...
    async def test_analyze_ecg_error_handling(self, ecg_service):
        ...
        with patch.object(ecg_service.ecg_reader, 'read_ecg') as mock_read:
            mock_read.return_value = {}  # Empty dict indicates error
    ...
    async def test_run_simplified_analysis_corrected(self, ecg_service):
        """Test simplified analysis - CORRECTED signature."""
    ...
    async def test_detect_pathologies_corrected(self, ecg_service):
        """Test pathology detection - CORRECTED signature."""
        signal_data = np.random.randn(1000, 12) * 0.1
...
class TestECGServiceIntegrationCorrected:
    ...
    async def test_complete_workflow_integration(self, ecg_service, tmp_path):
        ...
        result = await ecg_service.analyze_ecg_comprehensive(str(csv_file))
    ...
    async def test_performance_requirements(self, ecg_service):
        ...
        with patch.object(ecg_service.reader, 'read_ecg') as mock_read:
            mock_read.return_value = {'signal': test_signal, 'sampling_rate': 500}
            
            result = await ecg_service.analyze_ecg_comprehensive("test.csv")
...
class TestECGRegulatoryComplianceCorrected:
    ...
    async def test_metadata_compliance_requirements(self, ecg_service):
        ...
        assert 'software_version' in result['processing_metadata']
...
class TestECGAdditionalCoverageCorrected:
    """Additional tests to achieve 95%+ coverage with corrected signatures."""
    
    def test_feature_extractor_r_peaks_exception_corrected(self):
        """Test R peak detection exception handling - CORRECTED."""
    ...
    def test_feature_extractor_hrv_exception_corrected(self):
        """Test HRV feature extraction exception handling - CORRECTED."""
        extractor = FeatureExtractor()
    ...
    @pytest.mark.asyncio
    async def test_hybrid_service_simplified_analysis_exception_corrected(self):
        """Test simplified analysis exception handling - CORRECTED."""
        mock_db = Mock()
        mock_validation = Mock()
        ...
        with patch.object(ecg_service.feature_extractor, 'extract_all_features', side_effect=Exception("Feature error")):
            try:
                ...
                assert isinstance(e, ECGProcessingException)
    
    def test_hybrid_service_detect_pathologies_exception_corrected(self):
        """Test pathology detection exception handling - CORRECTED."""
        mock_db = Mock()
        mock_validation = Mock()
        ecg_service = HybridECGAnalysisService(db=mock_db, validation_service=mock_validation)
        ...
        with patch.object(ecg_service, '_detect_atrial_fibrillation', side_effect=Exception("AF detection error")):
            try:
                result = ecg_service._detect_pathologies(signal_data, features)
                assert isinstance(result, dict)
            except Exception as e:
                assert isinstance(e, ECGProcessingException)
    
    @pytest.mark.asyncio
    async def test_hybrid_service_clinical_assessment_exception_corrected(self):
        """Test clinical assessment exception handling - CORRECTED."""
        mock_db = Mock()
        mock_validation = Mock()
        ecg_service = HybridECGAnalysisService(db=mock_db, validation_service=mock_validation)
    ...
    @pytest.mark.asyncio
    async def test_hybrid_service_signal_quality_exception_corrected(self):
        """Test signal quality assessment exception handling - CORRECTED."""
        mock_db = Mock()
        mock_validation = Mock()
        ecg_service = HybridECGAnalysisService(db=mock_db, validation_service=mock_validation)
