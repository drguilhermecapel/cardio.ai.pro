...
def test_cors_origins():
    ...
    cors_origins = getattr(settings, 'BACKEND_CORS_ORIGINS', getattr(settings, 'ALLOWED_HOSTS', []))
...
def test_file_upload_settings():
    ...
    max_upload = getattr(settings, 'MAX_UPLOAD_SIZE', getattr(settings, 'MAX_ECG_FILE_SIZE', 0))
    assert max_upload > 0


def test_ai_model_settings():
    """Test AI model settings."""
    model_path = getattr(settings, 'AI_MODEL_PATH', getattr(settings, 'MODELS_DIR', None))
