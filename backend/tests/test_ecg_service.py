...
async def test_generate_report(ecg_service, test_db):
    ...
    with patch.object(ecg_service.repository, 'get_measurements_by_analysis') as mock_measurements, \
         patch.object(ecg_service.repository, 'get_annotations_by_analysis') as mock_annotations:
...
async def test_create_analysis_error_handling(ecg_service):
    ...
    ecg_service.repository.create_analysis = AsyncMock(side_effect=Exception("Database error"))
...
async def test_process_analysis_async_success(ecg_service):
    ...
    ecg_service.processor.load_ecg_data = AsyncMock(return_value=np.random.rand(5000, 12))
...
async def test_process_analysis_async_error_handling(ecg_service):
    ...
    ecg_service.processor.load_ecg_data = AsyncMock(side_effect=Exception("File processing error"))
...
async def test_generate_annotations_edge_cases(ecg_service):
    ...
    ai_results = {
        "events": [
            {"label": "pvc", "time_ms": 200, "confidence": 0.9, "properties": {"severity": "mild"}},
...
async def test_get_normal_range_comprehensive(ecg_service):
    ...
    measurement_types = ["heart_rate", "pr_interval", "qrs_duration", "qt_interval", "qtc_interval"]
