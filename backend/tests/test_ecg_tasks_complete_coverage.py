...
class TestECGTasksBasicFunctionality:
    ...
    def test_process_ecg_analysis_success(
        self, mock_current_task, mock_session_factory, mock_ecg_service
    ):
        """Test successful ECG analysis task execution."""
        mock_factory, mock_session = mock_session_factory

        with patch("app.tasks.ecg_tasks.MLModelService"), patch(
    ...
    def test_process_ecg_analysis_exception_handling(
        self, mock_current_task, mock_session_factory
    ):
        ...
        with patch("app.tasks.ecg_tasks.ECGAnalysisService") as mock_service_class:
            ...
            with patch("app.tasks.ecg_tasks.MLModelService"), patch(
                "app.services.notification_service.NotificationService"
            ), patch("app.tasks.ecg_tasks.ValidationService"), pytest.raises(
...
class TestECGTasksMedicalSafety:
    ...
    def test_critical_analysis_task_isolation(self, mock_current_task):
        ...
        with patch("app.tasks.ecg_tasks.get_session_factory") as mock_factory, patch(
            "app.tasks.ecg_tasks.ECGAnalysisService"
        ) as mock_service_class, patch("app.tasks.ecg_tasks.MLModelService"), patch(
            "app.services.notification_service.NotificationService"
        ), patch(
            "app.tasks.ecg_tasks.ValidationService"
        ):
...
class TestECGTasksPerformanceAndReliability:
    ...
    def test_service_dependency_injection(self, mock_current_task):
        ...
        with patch("app.tasks.ecg_tasks.get_session_factory") as mock_factory, patch(
            ...
            "app.tasks.ecg_tasks.MLModelService"
            ...
            mock_service_instance._process_analysis_async = AsyncMock(
                return_value={"status": "completed"}
            )
    ...
    def test_async_execution_context_management(self, mock_current_task):
        ...
        with patch("app.tasks.ecg_tasks.get_session_factory") as mock_factory, patch(
            ...
            "app.tasks.ecg_tasks.asyncio.run"
        ) as mock_asyncio_run:
