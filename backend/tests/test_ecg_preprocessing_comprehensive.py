...
# Assumindo que o módulo principal está disponível
...
class TestECGPreprocessing(unittest.TestCase):
    ...
    def test_clean_signal_processing(self):
        ...
        self.assertEqual(result.quality_metrics.quality_class, SignalQuality.EXCELLENT)
    ...
    def test_noisy_signal_improvement(self):
        ...
        quality_before = evaluate_signal_quality(ecg_noisy, self.fs)
        
        # Processar
        result = self.preprocessor.process(ecg_noisy)
        
        # Verificar melhoria
        improvement = result.quality_metrics.overall_score - quality_before['overall_score']
        self.assertGreater(improvement, 0.15)  # Melhoria mínima de 15%
        
        # SNR deve melhorar significativamente
        snr_improvement = result.quality_metrics.snr - quality_before['snr_db']
        self.assertGreater(snr_improvement, 10)  # Melhoria de pelo menos 10dB
    ...
    def test_quality_metrics_accuracy(self):
        ...
        quality = evaluate_signal_quality(ecg_wander, self.fs)
        self.assertGreater(quality['baseline_wander'], 0.5)
        
        # 2. Teste de interferência de linha
        ecg_powerline = ECGTestGenerator.add_powerline_interference(ecg_clean, 0.3)
        quality = evaluate_signal_quality(ecg_powerline, self.fs)
        self.assertGreater(quality['powerline_interference'], 0.1)
        
        # 3. Teste de SNR
        ecg_noisy = ECGTestGenerator.add_gaussian_noise(ecg_clean, 5)  # 5dB
        quality = evaluate_signal_quality(ecg_noisy, self.fs)
        self.assertLess(quality['snr_db'], 10)
...
if __name__ == "__main__":
    ...
    axes[1].set_title(f'ECG com Ruído (Qualidade: {evaluate_signal_quality(ecg_noisy, 360)["overall_score"]:.2%})')
