...
class TestAdvancedECGPreprocessor:
    ...
    def test_preprocessing_pipeline_basic(self):
        ...
        expected_keys = ['quality_score', 'r_peaks_detected', 'processing_time_ms', 
                        'segments_created', 'meets_quality_threshold']
    ...
    def test_quality_assessment(self):
        ...
        clean_quality = self.preprocessor._assess_signal_quality_realtime(self.clean_ecg)
        assert 0 <= clean_quality <= 1
        
        noisy_quality = self.preprocessor._assess_signal_quality_realtime(self.noisy_ecg)
    ...
    def test_segmentation_methods(self):
        ...
        segments = self.preprocessor._fixed_window_segmentation(self.clean_ecg, window_seconds=5)
...
class TestEnhancedSignalQualityAnalyzer:
    ...
    def test_comprehensive_quality_assessment(self):
        ...
        good_metrics = self.analyzer.assess_signal_quality_comprehensive(self.good_signal)
        
        expected_keys = ['overall_score', 'lead_scores', 'noise_characteristics', 
                        'artifact_detection', 'frequency_analysis', 'morphology_assessment', 
                        'recommendations']
        for key in expected_keys:
        ...
        poor_metrics = self.analyzer.assess_signal_quality_comprehensive(self.poor_signal)
    ...
    def test_noise_characterization(self):
        ...
        expected_keys = ['powerline_interference', 'baseline_wander', 'muscle_artifact', 
                        'electrode_noise', 'dominant_noise_type']
        for key in expected_keys:
    ...
    def test_artifact_detection(self):
        ...
        expected_keys = ['saturation_detected', 'flat_line_segments', 'sudden_jumps', 
                        'periodic_interference', 'lead_disconnection']
        for key in expected_keys:
            assert key in artifacts
    
    def test_frequency_analysis(self):
        """Test frequency domain analysis"""
        freq_analysis = self.analyzer._analyze_frequency_domain(self.good_signal.reshape(-1, 1))
        
        expected_keys = ['spectral_entropy', 'dominant_frequency', 'bandwidth_90', 
