...
class ECGDocumentScanner:
    ...
    async def digitize_ecg(self, filepath: str, sampling_rate: int = 500) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for _, region in enumerate(lead_regions):
                signal, confidence = self._extract_signal_from_region(region, sampling_rate)
    ...
    def _detect_grid(self, image: np.ndarray[Any, Any]) -> dict[str, Any]:
        ...
        grid_detected = (np.sum(horizontal_lines) > 1000 and np.sum(vertical_lines) > 1000)
