...
class ProductionMonitor:
    ...
    def log_prediction(self,
        ...
        try:
            ...
            if error is None:
                ...
                primary_diagnosis = prediction_result.get('primary_diagnosis', 'Unknown')

                prediction_log.update({
                    'confidence': confidence,
                    'primary_diagnosis': primary_diagnosis,
                    'predictions': prediction_result.get('predictions', {}),
                    'clinical_urgency': str(prediction_result.get('clinical_urgency', 'LOW'))
                })
                ...
                self.logger.error(f"Erro na predição para paciente {patient_id}: {error}")
    ...
    def _check_performance(self) -> None:
        ...
        try:
            ...
            recent_predictions = list(self.prediction_buffer)[-100:] if self.prediction_buffer else []
            recent_qualities = list(self.quality_buffer)[-100:] if self.quality_buffer else []
            recent_errors = list(self.error_buffer)[-10:] if self.error_buffer else []

            if recent_predictions:
                avg_confidence = np.mean([p.get('confidence', 0) for p in recent_predictions if p.get('confidence')])
                avg_processing_time = np.mean([p.get('processing_time', 0) for p in recent_predictions])
            else:
                avg_confidence = 0.0
            ...
            if avg_processing_time > self.alert_thresholds['max_processing_time']:
                alerts.append(f"Tempo de processamento alto: {avg_processing_time:.2f}s")

            if avg_quality < self.alert_thresholds['min_quality_score']:
                alerts.append(f"Qualidade de sinal baixa: {avg_quality:.3f}")

            if len(recent_errors) >= 5:
                alerts.append(f"Múltiplos erros recentes: {len(recent_errors)} nos últimos registros")
    ...
    def _send_alerts(self, alerts: list[str], metrics: dict[str, float]) -> None:
        ...
        try:
            ...
            if (last_alert and isinstance(last_alert, datetime) and
                now - last_alert < timedelta(minutes=cooldown_mins)):
                self.logger.info("Alerta suprimido devido ao cooldown")
                return

            alert_message = "🚨 ALERTA CardioAI Pro - Problemas de Performance Detectados\n\n"
            ...
            self.logger.info(f"Alerta enviado com {len(alerts)} problemas identificados")
    ...
    def get_performance_report(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_predictions = list(self.prediction_buffer)[-1000:] if self.prediction_buffer else []
            recent_errors = list(self.error_buffer)[-50:] if self.error_buffer else []

            overall_status = "HEALTHY"
            if (success_rate < self.alert_thresholds['min_success_rate'] or
                error_rate > self.alert_thresholds['max_error_rate'] or
                self.performance_metrics['average_confidence'] < self.alert_thresholds['min_confidence']):
            ...
            report = {
                ...
                'performance_metrics': {
                    ...
                    'average_quality_score': self.performance_metrics['quality_score_average']
                },
                'recent_activity': {
                ...
                'last_alert': self.performance_metrics['last_alert_time'].isoformat() if isinstance(self.performance_metrics['last_alert_time'], datetime) else None
            }
        ...
        try:
            for key, value in new_thresholds.items():
                if key in self.alert_thresholds:
                    ...
                    self.logger.info(f"Threshold {key} atualizado: {old_value} -> {value}")
                else:
