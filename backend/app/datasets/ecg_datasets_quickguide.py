...
def scenario_1_download_and_explore():
    ...
    stats = analyzer.analyze_dataset(records, "MIT-BIH Sample")
    ...
    for i, record in enumerate(records[:4]):
        ...
        ax.text(0.02, 0.95, f'FS: {record.sampling_rate} Hz', 
...
def scenario_2_prepare_ml_dataset(records=None):
    ...
    X, y = prepare_ml_dataset(
        records, 
        window_size=3600,  # 10 segundos @ 360Hz
...
def quick_visualize_ecg(record: ECGRecord, duration: float = 10.0):
    ...
    if len(record.signal.shape) > 1:
        for i in range(min(3, record.signal.shape[1])):
            plt.plot(t, record.signal[:samples, i] + i*2, 
