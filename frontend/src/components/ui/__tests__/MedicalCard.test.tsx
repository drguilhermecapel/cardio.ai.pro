...
describe('MedicalCard', () => {
  ...
  it('applies correct severity colors for normal state', () => {
    const { container } = render(
      <MedicalCard {...defaultProps} severity="normal" />
    )
  ...
  it('applies correct severity colors for warning state', () => {
    const { container } = render(
      <MedicalCard {...defaultProps} severity="warning" />
    )
  ...
  it('applies correct severity colors for critical state', () => {
    const { container } = render(
      <MedicalCard {...defaultProps} severity="critical" />
    )
  ...
  it('applies custom className when provided', () => {
    const { container } = render(
      <MedicalCard {...defaultProps} className="custom-medical-card" />
    )
  ...
  it('displays different trend icons correctly', () => {
    const { rerender } = render(
      <MedicalCard {...defaultProps} trend="up" trendValue="+5%" />
    )
    expect(screen.getByText('↗')).toBeInTheDocument()

    rerender(
      <MedicalCard {...defaultProps} trend="down" trendValue="-3%" />
    )
    expect(screen.getByText('↘')).toBeInTheDocument()

    rerender(
      <MedicalCard {...defaultProps} trend="stable" trendValue="0%" />
    )
    expect(screen.getByText('→')).toBeInTheDocument()
  })
