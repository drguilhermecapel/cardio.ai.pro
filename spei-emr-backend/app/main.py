...
async def ai_diagnose(
    request: DiagnosticRequest, current_user: str = Depends(verify_token)
):
    ...
    if any(symptom in symptoms for symptom in ["chest pain", "dor no peito", "angina"]):
        diagnoses.append(
            {"condition": "Angina Pectoris", "confidence": 0.85, "type": "primary"}
        )
        diagnoses.append(
            {
    ...
    if any(
        symptom in symptoms
        for symptom in ["shortness of breath", "dyspnea", "falta de ar"]
    ):
        diagnoses.append(
            {"condition": "Heart Failure", "confidence": 0.75, "type": "primary"}
        )
        diagnoses.append(
            {
    ...
    if any(symptom in symptoms for symptom in ["fever", "febre", "temperature"]):
        diagnoses.append(
            {"condition": "Viral Infection", "confidence": 0.70, "type": "primary"}
        )
        diagnoses.append(
            {
    ...
    treatment_plan = (
        "Consult with specialist for detailed evaluation and treatment planning."
    )
    ...
    return DiagnosticResponse(
        ...
        confidence_score=(
            max([d["confidence"] for d in diagnoses]) if diagnoses else 0.5
        ),
